...
function persistTravauxResult(input: TravauxSimulationV1, computed: ComputedTravaux): void {
  ...
  upsertInvestisseurProject(projectId, {
    execution: { travaux: { input, computed, updatedAt: new Date().toISOString() } },
  });
  addInvestisseurEvent({
...
function applyTravauxToAnalyse(
  ...
  patchExecutionTravaux({ input: simulation, computed, updatedAt: new Date().toISOString(), sourceMode });
  return true;
}
...
const PIECE_ICONS: Record<string, string> = Object.fromEntries(PIECE_TYPE_OPTIONS.map((o) => [o.value, o.icon]));
...
function createPieceUI(kind: PieceType = "sejour", surfaceM2 = 15): PieceUI {
  ...
  return { id: `piece_${Date.now()}_${_counter}`, kind, name: `${label} ${_counter}`, surfaceM2, ...defaultToggles(kind), ...defaultQtys(kind, surfaceM2) };
}

function pieceUIToTravaux(p: PieceUI): PieceTravaux {
  const items: ExpertLineItem[] = [];
  const push = (itemCode: PricingItemCode, qty: number) => { if (qty > 0) items.push({ itemCode, qty }); };
...
const DEFAULT_OPTIONS: TravauxOptionsSimple = {
  cuisineRefaire: "none", sdbRefaire: "none", electricite: "none",
  plomberie: "none", menuiseries: "none", isolationThermique: "none",
...
const SectionTitle: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <h3 className="text-sm font-semibold text-gray-700 uppercase tracking-wide mb-2">{children}</h3>
);
    ...
    <div className="flex rounded-lg border border-gray-200 overflow-hidden">
      {(["none", "partial", "full"] as TriChoice[]).map((opt) => (
        <button key={opt} type="button" onClick={() => onChange(opt)}
          className={`px-3 py-1 text-xs font-medium transition-colors ${value === opt ? "bg-violet-600 text-white" : "bg-white text-gray-500 hover:bg-gray-50"}`}>
          {TRI_LABELS[opt]}
        </button>
      ))}
    </div>
...
const ToggleRow: React.FC<{
  ...
  <div className="flex items-center gap-3 py-1">
    <div className={`w-9 h-5 rounded-full transition-colors relative cursor-pointer flex-shrink-0 ${checked ? "bg-violet-500" : "bg-gray-300"}`}
      onClick={() => onChange(!checked)}>
      <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform ${checked ? "translate-x-4" : "translate-x-0.5"}`} />
    </div>
    <span className="text-xs text-gray-700 w-[5.5rem] flex-shrink-0 select-none">
      {label}{hint && <span className="text-[10px] text-gray-400 ml-1">({hint})</span>}
    </span>
...
const ApplyToAnalyseButton: React.FC<{
  ...
  const handleClick = useCallback(() => {
    ...
    timerRef.current = setTimeout(() => { setStatus("idle"); timerRef.current = null; }, 2000);
...
const PieceCard: React.FC<{
  ...
  const updateSurface = (surfaceM2: number) => update({ surfaceM2, ...defaultQtys(p.kind, surfaceM2) });
  ...
  const quickTotal = travaux.items.reduce((sum, li) => sum + li.qty * getUnitPrice(li.itemCode, range), 0);
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          <select value={p.kind} onChange={(e) => updateKind(e.target.value as PieceType)}
            ...
            {PIECE_TYPE_OPTIONS.map((opt) => <option key={opt.value} value={opt.value}>{opt.icon} {opt.label}</option>)}
          </select>
          <input type="text" value={p.name} onChange={(e) => update({ name: e.target.value })}
            className="text-sm bg-transparent border-b border-gray-200 focus:border-violet-400 outline-none px-1 py-0.5 min-w-0 flex-1" placeholder="Nom" />
        </div>
        <div className="flex items-center gap-2 flex-shrink-0">
          <span className="text-sm font-bold text-violet-700">{fmtEuro(quickTotal)}</span>
          <button type="button" onClick={onRemove} className="text-gray-400 hover:text-red-500 transition-colors p-1" title="Supprimer">
      ...
      <div className="flex items-center gap-3">
        ...
        <input type="number" min={1} max={200} value={p.surfaceM2}
...
const PieceEditor: React.FC<{ pieces: PieceUI[]; onChange: (pieces: PieceUI[]) => void; range: TravauxRange }> = ({ pieces, onChange, range }) => {
  const handleUpdate = useCallback((id: string, updated: PieceUI) => onChange(pieces.map((p) => (p.id === id ? updated : p))), [pieces, onChange]);
  const handleRemove = useCallback((id: string) => onChange(pieces.filter((p) => p.id !== id)), [pieces, onChange]);
  const handleAdd = useCallback((kind: PieceType = "sejour") => onChange([...pieces, createPieceUI(kind, 15)]), [pieces, onChange]);

  return (
    <div className="space-y-3">
      {pieces.map((p) => (
        <PieceCard key={p.id} piece={p} range={range} onChange={(u) => handleUpdate(p.id, u)} onRemove={() => handleRemove(p.id)} />
      ))}
      <div className="flex flex-wrap gap-2 pt-1">
        ...
        <div className="relative group">
          <button type="button" className="flex items-center gap-1.5 px-3 py-2 text-xs font-medium text-gray-500 bg-gray-50 hover:bg-gray-100 border border-dashed border-gray-300 rounded-lg transition-colors">+ Autre…</button>
...
function computePieceBreakdown(items: ExpertLineItem[], range: TravauxRange): { groups: LotGroup[]; total: number } {
...
const PieceResultList: React.FC<{ pieces: PieceTravaux[]; range: TravauxRange }> = ({ pieces, range }) => {
  ...
  if (pieces.length === 0) return <p className="text-sm text-gray-400 italic py-4 text-center">Ajoutez des pièces pour voir le détail.</p>;
  return (
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-white rounded-xl border border-gray-200 p-4 text-center">
          ...
          {disabledCount > 0 && <p className="text-[10px] text-gray-400 mt-0.5">{disabledCount} lot{disabledCount > 1 ? "s" : ""} masqué{disabledCount > 1 ? "s" : ""}</p>}
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-4 text-center">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 text-center">
          ...
          <p className="text-xl font-bold text-gray-900 mt-1">{filteredCostPerM2 !== null ? fmtEuro(filteredCostPerM2) : "—"}</p>
        </div>
      </div>
                    ...
                    <tbody>
                      {lot.lines.map((line, idx) => (
                        <tr key={`${line.code}-${idx}`} className="border-t border-gray-100">
                          ...
                          <td className="py-1.5 text-right font-medium text-gray-900 whitespace-nowrap">{fmtEuro(line.amount)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
...
const SimulationTravauxPage: React.FC<Props> = ({ theme, breadcrumb }) => {
  ...
  const simulation = useMemo((): TravauxSimulationV1 => {
    ...
    const surfaceTotal = mode === "expert" ? pieces.reduce((acc, p) => acc + p.surfaceM2, 0) : surface;
    return { version: 1, mode, range, renovationLevel, surfaceTotalM2: surfaceTotal, options, complexity, pieces, updatedAt: new Date().toISOString() };
  ...
  // ── Sync travaux → Copilot context ───────────────────────────────────────
  useEffect(() => {
    if (result.totalWithBuffer <= 0) return;
    const surfaceTotal = simulation.surfaceTotalM2 > 0 ? simulation.surfaceTotalM2 : undefined;
    setActiveCopilotContext({
      renovation_cost_total:  result.totalWithBuffer,
      renovation_cost_per_m2: surfaceTotal ? Math.round(result.totalWithBuffer / surfaceTotal) : undefined,
      renovation_level:       simulation.renovationLevel,
      renovation_gamme:       simulation.range,
    });
  }, [result.totalWithBuffer, simulation.renovationLevel, simulation.range, simulation.surfaceTotalM2]);
    ...
    <div style={{ minHeight: "100vh", background: "#f1f5f9" }}>
      ...
      >
        <div style={{ position: "relative" }}>
          ...
          <div style={{ fontSize: 36, fontWeight: 600, color: "#fff", marginBottom: 10, lineHeight: 1.1, letterSpacing: "-0.025em" }}>Simulation Travaux</div>
