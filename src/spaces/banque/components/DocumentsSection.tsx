...
function emptyDoc(): DocumentItem {
  return { id: crypto.randomUUID(), nom: "", type: "autre", statut: "attendu" };
}
...
export default function DocumentsSection({ dossierId, dossier, refresh }: Props) {
  ...
  const completude = items.length
    ? Math.round((items.filter((d) => d.statut === "valide" || d.statut === "recu").length / items.length) * 100)
    : 0;
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Complétude : {completude}%</p>
        </div>
        <div className="flex items-center gap-3">
          {saved && <span className="text-sm text-green-600 font-medium">✓ Sauvegardé</span>}
          <button onClick={addItem} className="rounded-lg border border-slate-300 px-3 py-2 text-sm hover:bg-slate-50">
            + Document
          </button>
        </div>
      </div>
      ...
      <div className="w-full rounded-full bg-slate-100 h-2 mb-4">
        <div className="rounded-full bg-green-500 h-2 transition-all" style={{ width: `${completude}%` }} />
      </div>

      {items.length === 0 ? (
        <div className="rounded-xl border-2 border-dashed border-slate-200 p-8 text-center">
          <p className="text-sm text-slate-400">Ajoutez les documents requis pour ce dossier.</p>
        </div>
      ) : (
                ...
                return (
                  <tr key={doc.id} className="border-b border-slate-50 hover:bg-slate-50/50">
                    <td className="px-4 py-2">
                      <input value={doc.nom} onChange={(e) => updateItem(doc.id, "nom", e.target.value)}
                        placeholder="Nom du document" className="w-full border-0 bg-transparent text-sm focus:outline-none" />
                    </td>
                    <td className="px-4 py-2">
                      <select value={doc.type} onChange={(e) => updateItem(doc.id, "type", e.target.value)}
                        className="rounded border border-slate-200 px-2 py-1 text-xs">
                        {DOC_TYPES.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}
                      </select>
                    </td>
                    <td className="px-4 py-2">
                      <select value={doc.statut} onChange={(e) => updateItem(doc.id, "statut", e.target.value as DocStatus)}
                    ...
                    <td className="px-4 py-2">
                      <input value={doc.commentaire ?? ""} onChange={(e) => updateItem(doc.id, "commentaire", e.target.value)}
                        placeholder="—" className="w-full border-0 bg-transparent text-xs text-slate-500 focus:outline-none" />
                    </td>
                    <td className="px-2 py-2">
                      <button onClick={() => removeItem(doc.id)} className="text-xs text-red-400 hover:text-red-600">✕</button>
                    </td>
                  </tr>
                );
              })}
