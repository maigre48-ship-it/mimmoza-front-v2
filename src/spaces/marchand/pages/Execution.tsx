...
export default function MarchandExecution() {
  ...
  const [planningMode, setPlanningMode] =
    useState<"auto" | "manuel">(DEFAULT_PLANNING_MODE);
  const [manualPhases, setManualPhases] = useState<TimelinePhase[]>([
    ...DEFAULT_MANUAL_PHASES,
  ]);
  ...
  const stats = useMemo(() => {
    ...
    const endDay = tasks.length
      ? Math.max(
          ...tasks.map((t) => (t.startDay || 1) + (t.durationDays || 0) - 1),
        )
      : 0;
    ...
    const riskScore =
      tasks.length === 0
        ? 0
        : Math.round(
            (tasks.reduce((s, t) => s + (t.riskLevel || 1), 0) /
              tasks.length) *
              10,
          ) / 10;
    ...
    return (
      <div style={{ fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, sans-serif" }}>
        ...
        <SectionCard
          ...
          subtitle="Va dans Pipeline et sélectionne un deal."
        >
          <div style={{ color: "#64748b", fontSize: 13, lineHeight: 1.6 }}>
            Aucun deal n'est sélectionné. Une fois un deal actif, cette page se
            pré-remplira automatiquement.
          </div>
        </SectionCard>
      </div>
    );
        ...
        <SectionCard
          ...
          right={
            ...
            >
              ...
              {stats.doneCount} terminé · {stats.doingCount} en cours ·{" "}
              {stats.todoCount} à faire
            </div>
          }
        >
          <div style={{ overflowX: "auto" }}>
            ...
            >
              <thead>
                <tr>
                  <th style={{ textAlign: "left", fontSize: 12, color: "#64748b", padding: "0 10px" }}>
                    Statut
                  </th>
                  <th style={{ textAlign: "left", fontSize: 12, color: "#64748b", padding: "0 10px" }}>
                    Lot / Tâche
              ...
              <tbody>
                {tasks.length === 0 ? (
                  ...
                  tasks.map((t) => {
                    ...
                    const daysLabel = `J${t.startDay} → J${
                      t.startDay + t.durationDays - 1
                    }`;
                    const riskColor =
                      t.riskLevel === 1
                        ? "#065f46"
                      ...
                      <tr key={t.id}>
                        ...
                        <td style={{ padding: "0 10px", textAlign: "right" }}>
                          <input
                            ...
                            value={t.budget}
                            onChange={(e) =>
                              updateTask(t.id, { budget: Number(e.target.value) })
                            }
                            style={{
                              width: 130,
                              textAlign: "right",
                        ...
                        <td style={{ padding: "0 10px", textAlign: "right" }}>
                          <input
                            ...
                            value={t.paid}
                            onChange={(e) =>
                              updateTask(t.id, { paid: Number(e.target.value) })
                            }
                            style={{
                              width: 130,
                              textAlign: "right",
                        ...
                        <td style={{ padding: "0 10px", textAlign: "center" }}>
                          <select
                            ...
                            onChange={(e) =>
                              updateTask(t.id, {
                                riskLevel: Number(e.target.value) as 1 | 2 | 3,
                              })
                            }
                            style={{
                              padding: "8px 10px",
          ...
          >
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              ...
              <Select
                ...
                onChange={(v) =>
                  setNewTask((s) => ({ ...s, status: v as TaskStatus }))
                }
                options={[
              ...
              <Field
                label="Budget"
                value={Number(newTask.budget ?? 0)}
                onChange={(v) => setNewTask((s) => ({ ...s, budget: v }))}
                suffix="€"
                step={100}
              />
              ...
              <Field
                label="Payé"
                value={Number(newTask.paid ?? 0)}
                onChange={(v) => setNewTask((s) => ({ ...s, paid: v }))}
                suffix="€"
                step={100}
              />
              ...
              <Field
                label="Début (jour)"
                value={Number(newTask.startDay ?? 1)}
                onChange={(v) => setNewTask((s) => ({ ...s, startDay: v }))}
                suffix="J"
                step={1}
                min={1}
              />
              ...
              <Field
                label="Durée"
                value={Number(newTask.durationDays ?? 1)}
                onChange={(v) =>
                  setNewTask((s) => ({ ...s, durationDays: v }))
                }
                suffix="jours"
                step={1}
                min={1}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setGlobal((s) => ({ ...s, startDate: e.target.value }))
                  }
                  style={{
                    width: "100%",
                    padding: "10px 12px",
              ...
              <Field
                ...
                onChange={(v) =>
                  setGlobal((s) => ({ ...s, dailyHoldingCost: v }))
                }
                suffix="€"
                step={5}
                min={0}
