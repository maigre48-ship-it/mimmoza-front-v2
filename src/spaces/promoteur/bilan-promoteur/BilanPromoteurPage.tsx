  ...
  useEffect(() => {
    try {
      ...
      if (studyId && surfaceVendableM2 > 0 && computed.caTotal > 0) {
        patchBilan({ prix_foncier: computed.foncier, prix_revient_total: computed.coutTotal, ca_previsionnel: computed.caTotal, marge_nette: computed.marge, taux_marge_nette_pct: computed.margePct, fonds_propres: Math.round(feasibility.fondsPropres) || null, credit_promotion: null, taux_credit_pct: ass.financingRatePct, duree_mois: feasibility.dureeOperationMois || null, roi_pct: null, tri_pct: Number.isFinite(feasibility.triPromoteurPct) ? Number(feasibility.triPromoteurPct.toFixed(1)) : null, ai_narrative: null, ai_generated_at: null, notes: computed.notes.join(" | ") || null, done: true }).catch((e) => console.warn("[BilanPromoteurPage] patchBilan failed:", e));
      }
