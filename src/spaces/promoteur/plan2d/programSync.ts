...
function scaleBuildingToArea(b: Building2D, area: number): Building2D {
  ...
  if (cur <= 0.0001) {
    ...
    return { ...b, rect: { ...b.rect, width: s, depth: s }, lockedAreaM2: area };
  }
  const k = Math.sqrt(area / cur); // uniforme : conserve l'aspect ET le centre
  return {
    ...b,
    rect: { ...b.rect, width: b.rect.width * k, depth: b.rect.depth * k },
...
export function makeProgramBuilding(
  ...
  const side = Math.sqrt(Math.max(1, spec.areaM2));
  const off = index * side * 1.3; // cascade pour ne pas empiler les créations
