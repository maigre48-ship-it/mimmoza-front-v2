...
const AI_EXTRACT_ENDPOINT = "http://localhost:3000/api/plu-ai-extract-zone";
...
function isZoneMeaningful(z: PluRulesZoneRow): boolean {
  ...
  const fondParcelleValue = pickFirstNumber(
    ...
    impl?.recul_fond_parcelle_min_m,    reculs?.fond_parcelle?.min_m,
...
export default function PluFaisabilite(): React.ReactElement {
  ...
  const handleAiExtractZone = useCallback(async () => {
    ...
    try {
      ...
      const txt = await res.text();
      const rawData = safeJsonParse<AiExtractResponse>(txt);

      if (import.meta.env.DEV) {
        console.log("[PLU][AI_EXTRACT] response", rawData);
        console.log("[PLU][AI_EXTRACT] missing", rawData?.data?.missing);
      }

      if (!res.ok) {
        throw new Error(
          rawData?.error || `Erreur ${res.status}: ${txt.substring(0, 200)}`
        );
      }
      ...
      const isActuallyComplete = missingArray.length === 0;

      const normalizedAiData: AiExtractResultData = {
        completeness_ok: isActuallyComplete,
        missing: missingArray,
        confidence_score: toNumber(payload.confidence_score),
        error: safeString(payload.error),
        source: safeString(payload.source) ?? "AI_EXTRACTION",
        hauteur: payload.hauteur,
        stationnement: payload.stationnement,
        implantation: payload.implantation,
        emprise: payload.emprise,
        reculs: payload.reculs,
        ces: payload.ces,
        notes: Array.isArray(payload.notes) ? payload.notes : undefined,
        zone_libelle: safeString(payload.zone_libelle),
      };
      ...
      if (import.meta.env.DEV) {
        console.log("[PluFaisabilite] AI extraction success:", {
          endpoint: AI_EXTRACT_ENDPOINT,
          document_id: selectedDocumentId,
          zone_code: selectedZoneCode,
          ruleset_id: (rawData as AiExtractResponse).ruleset_id ?? null,
          completeness_ok: isActuallyComplete,
          missing_count: missingArray.length,
          finalRuleset,
        });
      }

    } catch (e: unknown) {
      const msg = e instanceof Error ? e.message : "Erreur lors de l'extraction IA.";
      setAiExtractStatus("error");
  ...
  const resolvedRuleset = useMemo<ResolvedPluRulesetV1 | null>(() => {
    // Priority: activeResolvedRuleset > merge(base, ai) > ai > base
    ...
    return baseResolved;
  }, [activeResolvedRuleset, baseResolvedRuleset, aiResolvedRuleset, selectedZoneCode, currentUserOverrides]);
  ...
  useEffect(() => {
    if (editPanelOpen && resolvedRuleset) {
      ...
      setEditImplantationEnLimite(
        resolvedRuleset.reculs.implantation_en_limite.autorisee === null ? "null" :
        resolvedRuleset.reculs.implantation_en_limite.autorisee ? "true" : "false"
      );
      ...
      const existingUserNote = resolvedRuleset.notes.find(n => n.startsWith("[Note utilisateur]"));
      if (existingUserNote) {
  ...
  const handleSaveOverrides = useCallback(() => {
    ...
    const overrides: UserOverrideValues = {
      reculs: {
        ...
        implantation_en_limite_autorisee: editImplantationEnLimite === "null" ? undefined :
          editImplantationEnLimite === "true" ? true : false,
      },
    ...
    if (overrides.reculs.limites_separatives_min_m === undefined) delete overrides.reculs.limites_separatives_min_m;
    if (overrides.reculs.fond_parcelle_min_m === undefined) delete overrides.reculs.fond_parcelle_min_m;
    if (overrides.reculs.implantation_en_limite_autorisee === undefined) delete overrides.reculs.implantation_en_limite_autorisee;
  ...
  }, [selectedDocumentId, communeInsee, selectedZoneCode, rulesStatus, rulesZones.length, resolvedRuleset]);
  ...
  return (
              ...
              {editPanelOpen && (
                <div className="edit-form">
                  <div className="edit-form-grid">
                    ...
                    <div className="edit-form-field">
                      <label className="edit-label">Hauteur max (m) <span className="optional-tag">optionnel</span></label>
                      <input
                        type="text"
                        className="edit-input"
                        value={editHauteur}
                    ...
                    <div className="edit-form-field">
                      <label className="edit-label">CES max (%) <span className="optional-tag">optionnel</span></label>
                      <input
                        type="text"
                        className="edit-input"
                        value={editCes}
                    ...
                    <div className="edit-form-field">
                      <label className="edit-label">Stationnement / logement <span className="optional-tag">optionnel</span></label>
                      <input
                        type="text"
                        className="edit-input"
                    ...
                    <div className="ai-extract-missing">
                      ...
                      <ul className="ai-extract-missing-list">
                        {aiExtractResult.data.missing.map((m, i) => {
                          ...
                          return (
                            <li key={i} className={isNonBlocking ? "ai-missing-non-blocking" : "ai-missing-blocking"}>
                              {m.replace(" (optionnel)", "")}
                              {isNonBlocking && <span className="ai-missing-tag">optionnel</span>}
                            </li>
                          );
                        })}
                      </ul>
                ...
                {rulesZones
                  ...
                  .map((z) => {
                    ...
                    if (useResolved) {
                      ...
                      implantationEnLimiteAutorisee = resolvedRuleset.reculs.implantation_en_limite.autorisee;
                      facadeAvantDisplay = formatResolvedFacade(resolvedRuleset.reculs.facades.avant);
                      facadeLateralesDisplay = formatResolvedFacade(resolvedRuleset.reculs.facades.laterales);
    ...
    impl?.recul_fond_parcelle_min_m,                        reculs?.fond_parcelle?.min_m,
                    ...
                    const zoneLibelle = useResolved && resolvedRuleset.zone_libelle 
                      ? resolvedRuleset.zone_libelle 
                      : getZoneLibelle(z);
                    const isDetectedZone = zonesMatch(z.zone_code, detectedZoneCode);
                    const hasAiData = aiExtractResult && zonesMatch(aiExtractResult.zone_code, z.zone_code) && aiExtractResult?.data;
                    ...
                    return (
                      <div
                        ...
                        className={`rule-zone-card ${isDetectedZone ? "rule-zone-card--detected" : ""} ${hasAiData ? "rule-zone-card--ai" : ""} ${hasUserOverrides ? "rule-zone-card--user" : ""}`}
                      >
                        <div className="rule-zone-header">
                          ...
                          <div className="rule-zone-meta">
                            <span className="confidence-pill" title="Score de confiance heuristique">
                        ...
                        <div className="rule-grid">
                          <div className="rule-cell">
                            ...
                            <div className="rule-value">{formatMaybeNumber(voirieValue, "m")}</div>
                          </div>
