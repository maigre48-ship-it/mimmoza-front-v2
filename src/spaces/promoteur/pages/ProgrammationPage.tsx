...
export interface ProgrammationSnapshot {
  ...
  niveaux:        number;
  nbLogements:    number;          // somme des types
  empriseSol:     number;
...
function KpiCard({ label, value, sub, accent }: {
  ...
  return (
    <div style={{
      ...
      {sub && (
        <p style={{ margin: "3px 0 0", fontSize: 11, color: "#9CA3AF" }}>{sub}</p>
      )}
    </div>
  );
}
  ...
  return (
    <div style={{ marginBottom: 10 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
        ...
        />
        {unit && (
          <span style={{ fontSize: 12, color: "#9CA3AF", minWidth: 38, textAlign: "left" }}>{unit}</span>
        )}
      </div>
    </div>
  );
...
function ConclusionCard({ status, reasons }: { status: ViabiliteStatus; reasons: string[] }) {
  ...
  return (
    <div style={{
      background: s.bg, border: `2px solid ${s.border}`, borderRadius: 10, padding: "18px 22px",
    }}>
      <p style={{ margin: "0 0 10px", fontSize: 18, fontWeight: 800, color: s.text }}>
        {s.icon}&nbsp; {s.title}
      </p>
      <ul style={{ margin: 0, paddingLeft: 20 }}>
        {reasons.map((r, i) => (
          <li key={i} style={{ fontSize: 13, color: s.text, marginBottom: 4 }}>{r}</li>
        ))}
      </ul>
    </div>
  );
...
export default function ProgrammationPage() {
  ...
  // ── Terrain — depuis study.foncier (Supabase) → getSnapshot().foncier (localStorage) ──
  ...
  const terrain = useMemo(() => {
    const f = foncierData;
    if (!f) return {
      surfaceM2: 0, prixVendeur: 0, codePostal: "",
      commune: "", typeBien: "", adresse: "", pluDisponible: false,
    };

    // Commune — cherche en direct, puis dans le feature GeoJSON cadastral
    ...
    // Code postal — peut être dans le feature GeoJSON des parcelles
    const codePostal =
      f.code_postal ?? f.codePostal ?? f.cp ?? f.zip ??
      f.parcels_raw?.[0]?.feature?.properties?.code_postal ?? "";

    // Source / adresse
    const adresse =
      f.adresse ?? f.address ?? f.label ?? f.source_label ??
      f.libelle ?? f.adresse_complete ?? f.focus_id ?? "";

    // Type de bien
    const typeBien =
      f.type_bien ?? f.typeBien ?? f.type ?? f.nature ?? "Terrain";

    // Prix
    const prixVendeur =
      f.prix_foncier ?? f.prixVendeur ?? f.prix_vendeur ??
      f.prix ?? f.price ?? f.valeur ?? 0;
  ...
  // ── PLU — depuis study.plu → getSnapshot().plu ──
  const plu = useMemo(() => {
    ...
    if (!p) return {
      ...
      reculVoirie: null, reculVoirieNote: null,
      reculLimites: null, reculLimitesNote: null,
      parkingParLogement: 1, espaceVertMinPct: 0,
      pleineTerreMinPct: null, coefficientOccupSol: null, cosNote: null,
      extra: [] as Array<{ label: string; value: string | null; note: string | null }>,
    };

    const rs = p.ruleset ?? p;

    /** Extrait la valeur numérique d'un objet règle PLU quelle que soit la clé utilisée */
    function numFrom(obj: any): number | null {
      if (obj == null) return null;
      if (typeof obj === "number") return obj;
      if (typeof obj === "string") { const n = parseFloat(obj); return isNaN(n) ? null : n; }
      for (const k of [
        "valeur","m","metres","max","min","min_pct","pct","pourcentage","percent",
        "max_ratio","min_m","valeur_m","distance","ratio","value","v",
      ]) {
    ...
    // Reculs — les clés peuvent être "voirie", "limites", "limites_separatives", etc.
    ...
    // Stationnement — peut être un objet ou un nombre direct
    const parkingParLogement = numFrom(stat) ?? numFrom(stat.par_logement) ??
      numFrom(p.parking_par_logement) ?? 1;

    // Zones extra : autres clés du ruleset non déjà couvertes
    const HANDLED = new Set([
      "ces","cos","reculs","recul","stationnement","parking",
      "hauteurs","gabarit","hauteur",          // gérés explicitement via hauteurNote
      "pleine_terre","espaces_verts","espace_vert", // gérés via pleineTerreMinPct
    ]);
    ...
    // Aplatit récursivement un objet ruleset en lignes {label, value, note}
    function flattenEntry(key: string, val: any): Array<{ label: string; value: string | null; note: string | null }> {
      ...
      // Si c'est un primitif directement
      const directNum = numFrom(val);
      if (directNum != null) return [{ label: baseLabel, value: String(directNum), note: noteFrom(val) }];
      if (typeof val !== "object") return [];
      // Cherche les sous-clés qui sont elles-mêmes des objets (ex: hauteurs.egout, hauteurs.faitage)
    ...
    return {
      ...
      hauteurEgoutM:      numFrom(hauteurs.egout)     ?? numFrom(hauteurs.egout_m)    ??
                          numFrom(hauteurs.egout_max) ?? numFrom(hauteurs.hauteur_egout) ??
                          numFrom(hauteurs.max)       ?? numFrom(p.hauteur_egout_m)    ?? null,
      hauteurFaitageM:    numFrom(hauteurs.faitage)   ?? numFrom(hauteurs.faitage_m)  ??
                          numFrom(hauteurs.faitage_max) ?? numFrom(p.hauteur_faitage_m) ?? null,
      hauteurMaxM:        numFrom(hauteurs.max)       ?? numFrom(p.hauteur_max_m)     ?? 0,
      hauteurNote:        noteFrom(hauteurs) ?? null,   // ex: "10m égout, 13m faîtage"
      // Si hauteurEgoutM n'a pas été trouvé directement, on l'extrait de la note
  ...
  // Surface par type (m²) — valeurs de référence marché IDF
  ...
  // nbLogements dérivé du détail typologies
  ...
  const calculs = useMemo(() => {
    ...
    const prixTerrainM2  = terrain.surfaceM2 > 0
      ? terrain.prixVendeur / terrain.surfaceM2 : 0;
    const chargeFonciere = sdpTotale > 0
      ? terrain.prixVendeur / sdpTotale : 0;
    ...
    return {
      sdpLogement, sdpCommerce, sdpTotale, hauteurEstimeeM,
      prixTerrainM2, chargeFonciere,
      caLogement, caCommerce, caTotal,
      coutTravauxLog, coutTravauxCom, coutTravaux, fraisAnnexes, coutTotal,
      margeBrute, tauxMarge,
    };
  }, [
    nbT1, nbT2, nbT3, nbT4, nbT5, surfT1, surfT2, surfT3, surfT4, surfT5,
    showCommerce, surfaceCommerce, niveaux,
    terrain, prixVenteLog, prixVenteCom, coutConstrLog, coutConstrCom, fraisPct,
  ]);
  ...
  const pluChecks = useMemo((): PluCheck[] => {
    ...
    const emprisePctProjet = terrain.surfaceM2 > 0
      ? (empriseSol / terrain.surfaceM2) * 100 : 0;
    ...
    function checkEmprise(): ComplianceStatus {
      if (plu.empriseMaxPct === 0) return "a_verifier"; // pas de règle définie
    ...
    return [
      {
        critere:       "Emprise au sol",
        valeurProjet:  fmtPct(emprisePctProjet) + ` (${fmt(empriseSol)} m²)`,
        valeurPLU:     plu.empriseMaxPct > 0 ? `max. ${fmtPct(plu.empriseMaxPct)}` : "Pas de règle CES",
        status:        checkEmprise(),
      },
      {
        critere:       "Hauteur du projet",
        valeurProjet:  `${calculs.hauteurEstimeeM.toFixed(1)} m (${niveaux} niv.)`,
        valeurPLU:     plu.hauteurEgoutM != null ? `max. ${plu.hauteurEgoutM} m (égout)` : plu.hauteurMaxM > 0 ? `max. ${plu.hauteurMaxM} m` : "—",
        status:        checkHauteur(),
      },
      {
        critere:       "Stationnement",
        valeurProjet:  `${nbParkings} place${nbParkings > 1 ? "s" : ""}`,
        valeurPLU:     `min. ${parkingsRequis} (${plu.parkingParLogement} pl/logt)`,
        status:        checkParkings(),
      },
      {
        critere:       "Pleine terre / espaces verts",
        valeurProjet:  `${fmt(espacesVerts)} m²`,
        valeurPLU:     espaceVertMin > 0 ? `min. ${fmt(Math.round(espaceVertMin))} m² (${plu.pleineTerreMinPct ?? plu.espaceVertMinPct} %)` : "Pas de règle",
        status:        checkVerts(),
      },
    ];
  ...
  const { viabilite, reasons } = useMemo((): { viabilite: ViabiliteStatus; reasons: string[] } => {
    ...
    for (const check of pluChecks) {
      if (check.status === "non_conforme") {
        blocages++;
        msgs.push(`PLU — ${check.critere} non conforme : ${check.valeurProjet} (règle : ${check.valeurPLU})`);
      } else if (check.status === "a_verifier") {
        warnings++;
        msgs.push(`PLU — ${check.critere} à vérifier : ${check.valeurProjet} (règle : ${check.valeurPLU})`);
      }
    }

    if (!terrain.pluDisponible) {
      warnings++;
      msgs.push("PLU non chargé — le contrôle réglementaire reste à confirmer");
    }

    if (calculs.margeBrute <= 0) {
      blocages++;
      msgs.push(`Marge brute négative : ${fmtEur(calculs.margeBrute)} — projet déficitaire`);
    } else if (calculs.tauxMarge < margeCiblePct) {
      warnings++;
      msgs.push(`Marge ${fmtPct(calculs.tauxMarge)} inférieure à la cible de ${fmtPct(margeCiblePct)}`);
    } else {
      msgs.push(`Marge ${fmtPct(calculs.tauxMarge)} — cible ${fmtPct(margeCiblePct)} atteinte`);
    }

    const detailTotal = nbT1 + nbT2 + nbT3 + nbT4 + nbT5;
    if (detailTotal === 0) {
      warnings++;
      msgs.push("Aucun logement saisi — renseignez au moins un type de logement");
    }

    const sdpEmprise = empriseSol * niveaux;
    if (calculs.sdpTotale > sdpEmprise * 1.15) {
      warnings++;
      msgs.push(`SDP totale (${fmt(Math.round(calculs.sdpTotale))} m²) dépasse l'emprise × niveaux (${fmt(Math.round(sdpEmprise))} m²)`);
    }

    const status: ViabiliteStatus = blocages > 0 ? "non_viable" : warnings > 0 ? "conditions" : "viable";
    return { viabilite: status, reasons: msgs };
  }, [
    pluChecks, terrain.pluDisponible, calculs, margeCiblePct,
    nbT1, nbT2, nbT3, nbT4, nbT5, empriseSol, niveaux,
  ]);
  ...
  const handleSaveForBilan = () => {
    const snapshot: ProgrammationSnapshot = {
      ...
      nbParkings, espacesVerts,
      prixVenteLog, prixVenteCom, coutConstrLog, coutConstrCom, fraisPct, margeCiblePct,
      sdpLogement:  calculs.sdpLogement,
      sdpCommerce:  calculs.sdpCommerce,
      sdpTotale:    calculs.sdpTotale,
      caTotal:      calculs.caTotal,
      caLogement:   calculs.caLogement,
      caCommerce:   calculs.caCommerce,
      coutTravaux:  calculs.coutTravaux,
      coutTotal:    calculs.coutTotal,
      margeBrute:   calculs.margeBrute,
      tauxMarge:    calculs.tauxMarge,
  ...
  return (
    <div style={{ padding: "32px 28px", maxWidth: 1120, margin: "0 auto", fontFamily: "inherit" }}>
      ...
      <div style={{ marginBottom: 24, display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16 }}>
        <div>
          ...
          <p style={{ margin: "4px 0 0", fontSize: 14, color: "#6B7280" }}>
            Définissez votre programme et vérifiez la viabilité avant de passer en faisabilité.
          </p>
          {!terrain.pluDisponible && (
            <div style={{
              marginTop: 10, display: "inline-flex", alignItems: "center", gap: 8,
              padding: "7px 14px", borderRadius: 8, fontSize: 13, color: "#B45309",
              background: "#FFFBEB", border: "1px solid #FDE68A",
            }}>
              ⚠️ PLU non encore chargé — simulation provisoire basée sur valeurs par défaut.
            </div>
          )}
        </div>

        {/* Bouton envoi vers Bilan */}
        <button
        ...
        >
          {savedForBilan
            ? <><CheckCircle size={15} />Envoyé au bilan</>
            : <><Target size={15} />Valider & envoyer au bilan</>
          }
        </button>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 20 }}>
        ...
        <SectionCard title="Terrain analysé">
          ...
          <FieldRow
            label="Prix terrain au m²"
            value={terrain.surfaceM2 > 0 && terrain.prixVendeur > 0
              ? `${fmt(Math.round(terrain.prixVendeur / terrain.surfaceM2))} €/m²`
              : "—"}
          />
          ...
          {terrain.pluDisponible ? (
            <>
              ...
              {plu.description ? (
                <div style={{ fontSize: 12, color: "#6B7280", padding: "2px 0 8px", fontStyle: "italic" }}>
                  {plu.description}
                </div>
              ) : null}
              {(plu.hauteurEgoutM != null || plu.hauteurEgoutFromNote != null) && (
                <FieldRow
                  label="Hauteur max. (égout)"
                  value={`${plu.hauteurEgoutM ?? plu.hauteurEgoutFromNote} m`}
                />
              )}
              {plu.hauteurFaitageM != null && (
                <FieldRow label="Hauteur faîtage" value={`${plu.hauteurFaitageM} m`} />
              )}
              {plu.hauteurEgoutM == null && plu.hauteurEgoutFromNote == null && plu.hauteurFaitageM == null && plu.hauteurMaxM > 0 && (
                <FieldRow label="Hauteur max." value={`${plu.hauteurMaxM} m`} />
              )}
              <FieldRow
                label="Emprise au sol (CES)"
                value={plu.empriseMaxPct > 0 ? `${plu.empriseMaxPct} %` : "—"}
                note={plu.empriseNote ?? (plu.empriseMaxPct === 0 ? "Pas de règle" : undefined)}
              />
              {plu.reculVoirie != null && (
                <FieldRow label="Recul voirie" value={`${plu.reculVoirie} m`} note={plu.reculVoirieNote ?? undefined} />
              )}
              {plu.reculLimites != null && (
                <FieldRow label="Recul limites" value={`${plu.reculLimites} m`} note={plu.reculLimitesNote ?? undefined} />
              )}
              <FieldRow label="Stationnement" value={`${plu.parkingParLogement} pl/logt`} />
              {plu.pleineTerreMinPct != null && (
                <FieldRow label="Pleine terre min." value={`${plu.pleineTerreMinPct} %`} note={plu.pleineTerreNote ?? undefined} />
              )}
              {plu.espaceVertMinPct > 0 && plu.pleineTerreMinPct == null && (
                <FieldRow label="Espaces verts min." value={`${plu.espaceVertMinPct} %`} />
              )}
              <FieldRow
                label="COS"
                value={plu.coefficientOccupSol != null ? String(plu.coefficientOccupSol) : "—"}
                note={plu.cosNote ?? (plu.coefficientOccupSol == null ? "Pas de COS" : undefined)}
              />
              {/* Toutes les règles restantes du ruleset — affichage générique */}
              {plu.extra.map(e => (
                <FieldRow
                  key={e.label}
                  label={e.label}
        ...
        <SectionCard title="Programme immobilier">
          ...
          <SelectInput<string>
            ...
            value={String(niveaux)}
            onChange={(v) => setNiveaux(Number(v))}
          ...
          <p style={{ margin: "0 0 8px", fontSize: 11, color: "#9CA3AF" }}>
            Renseignez le nombre de logements et la surface habitable par type.
          </p>
          ...
          <div style={{ border: "1px solid #E5E7EB", borderRadius: 8, overflow: "hidden", marginBottom: 8 }}>
            {/* Header */}
            <div style={{
              display: "grid", gridTemplateColumns: "56px 1fr 1fr 72px",
              background: "#F5F3FF", padding: "7px 12px",
              fontSize: 11, fontWeight: 700, color: "#7C3AED",
              textTransform: "uppercase" as const, letterSpacing: "0.04em",
            }}>
              <span>Type</span>
              <span>Nb logements</span>
              <span>Surface (m²)</span>
              <span style={{ textAlign: "right" as const }}>SDP</span>
            </div>
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    background: isActive ? "#FDFCFF" : "white",
                  }}
                >
                  <span style={{ fontSize: 13, fontWeight: 700, color: isActive ? "#7C3AED" : "#9CA3AF" }}>
                    {key}
                  </span>
                  <div style={{ paddingRight: 10 }}>
                    <input
                      type="number" value={nb} min={0}
                      ...
                      onFocus={e => (e.currentTarget.style.borderColor = "#7C3AED")}
                      onBlur={e  => (e.currentTarget.style.borderColor = isActive ? "#C4B5FD" : "#E5E7EB")}
                      placeholder="0"
                    />
                  </div>
                  <div style={{ paddingRight: 10, display: "flex", alignItems: "center", gap: 6 }}>
                    <input
                      type="number" value={surf} min={10} step={1}
                      onChange={e => setSurf(Number(e.target.value))}
                      style={{
                  ...
                  <span style={{
                    ...
                    color: isActive ? "#5B21B6" : "#D1D5DB",
                    textAlign: "right" as const,
                  }}>
                    {isActive ? `${Math.round(sdp)} m²` : "—"}
                  </span>
                </div>
              );
            })}
            ...
            <div style={{
              ...
              background: "#EDE9FE", borderTop: "2px solid #DDD6FE",
            }}>
              ...
              <span style={{ fontSize: 11, color: "#7C3AED" }}>
                {nbLogements > 0
                  ? `moy. ${(calculs.sdpLogement / nbLogements).toFixed(0)} m²`
                  : "—"}
              </span>
              <span style={{ fontSize: 13, fontWeight: 800, color: "#5B21B6", textAlign: "right" as const }}>
                {Math.round(calculs.sdpLogement)} m²
              </span>
            </div>
          </div>
          ...
          {showCommerce && (
            <>
              <SubHeading label="Commerce" />
              <NumberInput label="Surface commerce" value={surfaceCommerce} onChange={setSurfaceCommerce} unit="m²" min={0} />
            </>
          )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "280px 1fr", gap: 20, marginBottom: 20 }}>
        ...
        <SectionCard title="Hypothèses économiques">
          ...
          {showCommerce && (
            <NumberInput label="Prix vente commerce"    value={prixVenteCom}  onChange={setPrixVenteCom}  unit="€/m²" min={500}   step={50} />
          )}
          <NumberInput label="Coût constr. logement"    value={coutConstrLog} onChange={setCoutConstrLog} unit="€/m²" min={500}   step={50} />
          {showCommerce && (
            <NumberInput label="Coût constr. commerce"  value={coutConstrCom} onChange={setCoutConstrCom} unit="€/m²" min={400}   step={50} />
          )}
        ...
        <SectionCard title="Résultats calculés">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10 }}>
            ...
            <KpiCard
              label="Marge brute"
              value={fmtEur(calculs.margeBrute)}
              accent={calculs.margeBrute > 0}
            />
            <KpiCard
              label="Taux de marge"
              value={fmtPct(calculs.tauxMarge)}
              sub={`Cible : ${fmtPct(margeCiblePct)}`}
              accent={calculs.tauxMarge >= margeCiblePct && calculs.tauxMarge > 0}
            />
          </div>
        </SectionCard>
      </div>
      ...
      <SectionCard title="Contrôle PLU" style={{ marginBottom: 20 }}>
        {!terrain.pluDisponible ? (
          <div style={{
            ...
            fontSize: 14, background: "#F9FAFB", borderRadius: 8,
          }}>
