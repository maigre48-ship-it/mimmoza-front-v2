// MassingEditor3D.tsx — V2.5
// V2.5 : ajout prop onUpdateRenderSpec → BuildingPropertiesPanel

import React, { type FC, useEffect, useMemo, useRef, useState } from "react";
import type { Feature, FeatureCollection, Polygon, MultiPolygon } from "geojson";

import { useMassingEditor, makeNewBuilding } from "../useMassingEditor";
import { MassingRenderer, type VegetationOptions } from "./MassingRenderer";
...
import type {
  MassingSceneModel, EditorTool, MassingBuildingModel,
  BuildingTemplateType, PlacedObject,
} from "../massingScene.types";
import { totalHeightM, totalLevelsCount } from "../massingScene.types";
import { patchFromTemplateType } from "../massingTemplates";
...
function convertBuildings2D(
  ...
  parcelRef?: { lon: number; lat: number } | null,
  ...
  buildings2D.forEach((b, idx) => {
    ...
    if (parcelRef) {
      const mPerDegLon = 111000 * Math.cos(parcelRef.lat * Math.PI / 180);
      const bldLon     = parcelRef.lon + cx / mPerDegLon;
      const bldLat     = parcelRef.lat - cy / 111000;
      const widthDeg   = w / mPerDegLon;
      const depthDeg   = d / 111000;

      if (idx === 0) {
        console.log(`[Massing3D] Bât ${idx} → WGS84: lon=${bldLon.toFixed(5)} lat=${bldLat.toFixed(5)} w=${widthDeg.toFixed(6)}° d=${depthDeg.toFixed(6)}°`);
      }

      ring = rectToRing({ center: { x: bldLon, y: bldLat }, width: widthDeg, depth: depthDeg, rotationDeg: rotDeg });
    } else {
      const dimMeters  = w >= 0.5 && w <= 2000;
      const ctrWgs84   = Math.abs(cx) <= 180 && Math.abs(cy) <= 90;
      if (dimMeters && ctrWgs84) {
        const mPerDegLon = 111000 * Math.cos(cy * Math.PI / 180);
        ring = rectToRing({ center: { x: cx, y: cy }, width: w / mPerDegLon, depth: d / 111000, rotationDeg: rotDeg });
      } else {
...
function convertParkings2D(
  ...
  parkings2D.forEach(p => {
    ...
    if (parcelRef && w <= 2000) {
      const mPerDegLon = 111000 * Math.cos(parcelRef.lat * Math.PI / 180);
      const pLon       = parcelRef.lon + cx / mPerDegLon;
      const pLat       = parcelRef.lat - cy / 111000;
      const wDeg       = w / mPerDegLon;
      const dDeg       = d / 111000;
      ring = rectToRing({ center: { x: pLon, y: pLat }, width: wDeg, depth: dDeg, rotationDeg: rotDeg });
    } else {
...
function getParcelCentroid(
  ...
  try {
    ...
    const lons = (ring as number[][]).map(c => c[0]);
    const lats = (ring as number[][]).map(c => c[1]);
    return {
...
const TOOLS: { tool: EditorTool; icon: string; label: string; tip: string }[] = [
  { tool: "select",   icon: "↖",  label: "Sélection", tip: "Clic sur un bâtiment" },
  { tool: "add_rect", icon: "＋□", label: "Ajouter",   tip: "Nouveau volume" },
  { tool: "orbit",    icon: "🎥", label: "Caméra",    tip: "Navigation libre" },
];

const ToolBtn: FC<{ active: boolean; icon: string; label: string; tip: string; onClick: () => void }> = ({
  active, icon, label, tip, onClick,
}) => (
  <button title={tip} onClick={onClick} style={{
    ...
    background: active ? "rgba(82,71,184,0.09)"  : "white",
...
export const MassingEditor3D: FC<MassingEditor3DProps> = ({
  ...
  height = "640px", className, onSceneChange,
}) => {
  ...
  const massingBldFrom2D = useMemo(
    () => convertBuildings2D(plan2D.buildings, buildingHeightM, getParcelCentroid(parcel)),
    [plan2D.buildings, buildingHeightM, parcel],
  );

  const parkingsFrom2D = useMemo(
    () => convertParkings2D(plan2D.parkings, getParcelCentroid(parcel)),
    [plan2D.parkings, parcel],
  );
  ...
  if (process.env.NODE_ENV === "development") {
    console.log("[Massing3D] effectiveParkings features:", effectiveParkings?.features?.length ?? 0,
      "parkingsProp:", parkingsProp?.features?.length ?? 0,
      "parkingsFrom2D:", parkingsFrom2D?.features?.length ?? 0);
  }
  ...
  }, [massingBldFrom2D, buildingsProp, parcel]);

  const handleResync = () => {
    const fresh = readPlan2D();
    setPlan2D(fresh);
    const newBlds = convertBuildings2D(fresh.buildings, buildingHeightM, getParcelCentroid(parcel));
    if (newBlds.length > 0) {
      replaceScene({ version: 1, buildings: newBlds, placedObjects: scene.placedObjects ?? [] });
    }
  };
  ...
  useEffect(() => {
    ...
    if (!parcel) return;
    let cancel = false;
    setReliefStatus("loading"); setReliefError(null); setFetchedRelief(null);
    (async () => {
      try {
        const result = await fetchReliefFromEdgeFunction(parcel);
        if (cancel) return;
        ...
        setFetchedRelief({ elevations: result.elevations, nx: result.nx, ny: result.ny, dx, dy, minZ: result.minZ, maxZ: result.maxZ, bbox: result.bbox });
        setReliefStatus("ready");
        setReliefMeta({ deltaZ: dz });
      } catch (e: any) {
        if (!cancel) { setReliefStatus("error"); setReliefError(e?.message ?? String(e)); }
      }
    })();
    return () => { cancel = true; };
  }, [parcel, reliefDataProp]);

  const [vegetation,    setVegetation]    = useState<VegetationOptions>({ showHedges: false, showTrees: false, showBushes: false, treeType: "deciduous", treeSpacing: 8, hedgeHeight: 1.2 });
  ...
  const handleAddBuilding = () => {
    if (has2DData) { handleResync(); return; }
    let cx = 0, cy = 0;
    if (parcel) {
      const ring = parcel.geometry.type === "Polygon"
        ? parcel.geometry.coordinates[0]
        : parcel.geometry.coordinates[0]?.[0];
      if (ring) { for (const [x, y] of ring as number[][]) { cx += x; cy += y; } cx /= ring.length; cy /= ring.length; }
    }
    const isGeo = Math.abs(cx) < 180, mPerU = isGeo ? 111000 : 1;
    const halfW = 15 / mPerU / 2, halfD = 10 / mPerU / 2;
    const rect: [number, number][] = [
      [cx - halfW, cy - halfD], [cx + halfW, cy - halfD],
      [cx + halfW, cy + halfD], [cx - halfW, cy + halfD],
      [cx - halfW, cy - halfD],
    ];
    const bld = makeNewBuilding({ name: `Bâtiment ${scene.buildings.length + 1}`, footprint: { points: rect, epsg: isGeo ? "4326" : "2154" } });
    const newId = addBuilding(bld);
  ...
  const bc = reliefStatus === "ready"
    ...
    ? { bg: "rgba(234,179,8,0.12)",   border: "rgba(234,179,8,0.4)",  text: "#a16207" }
    : reliefStatus === "error"
    ? { bg: "rgba(239,68,68,0.10)",   border: "rgba(239,68,68,0.3)",  text: "#dc2626" }
    : { bg: "rgba(255,255,255,0.80)", border: "#e2e8f0",               text: "#64748b" };
  ...
  return (
    <div className={className} style={{
      ...
      <div style={{ flex: 1, position: "relative", minWidth: 0 }}>
        ...
        <div style={{ position: "absolute", top: 10, left: 10, zIndex: 10, display: "flex", gap: 4, background: "rgba(255,255,255,0.92)", padding: 4, borderRadius: 10, backdropFilter: "blur(8px)", boxShadow: "0 2px 8px rgba(0,0,0,0.08)" }}>
          {TOOLS.map(t => (
            <ToolBtn key={t.tool} active={activeTool === t.tool} icon={t.icon} label={t.label}
              tip={t.tool === "add_rect" && has2DData ? "Re-synchroniser depuis l'implantation 2D" : t.tip}
              onClick={() => t.tool === "add_rect" ? handleAddBuilding() : setTool(t.tool)} />
          ))}
        </div>

        <div style={{ position: "absolute", top: 10, right: 10, zIndex: 10, padding: "4px 10px", borderRadius: 20, fontSize: 10, fontWeight: 600, background: bc.bg, border: `1px solid ${bc.border}`, color: bc.text, backdropFilter: "blur(8px)", boxShadow: "0 1px 4px rgba(0,0,0,0.06)", maxWidth: 300, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={reliefError ?? undefined}>
          {reliefStatus === "idle"    && "🏔 Relief —"}
          {reliefStatus === "loading" && "🏔 Chargement terrain…"}
        ...
        <div style={{ position: "absolute", bottom: 10, left: 10, zIndex: 10, display: "flex", gap: 6, fontSize: 10 }}>
          <label style={{ display: "flex", alignItems: "center", gap: 4, padding: "4px 8px", borderRadius: 6, background: "rgba(255,255,255,0.88)", cursor: "pointer", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
            <input type="checkbox" checked={showWireframe} onChange={e => setShowWireframe(e.target.checked)} style={{ accentColor: ACCENT }} /> Fil de fer
          </label>
          {hasTerrain && (
            <label style={{ display: "flex", alignItems: "center", gap: 4, padding: "4px 8px", borderRadius: 6, background: showTerrain ? "rgba(82,71,184,0.10)" : "rgba(255,255,255,0.88)", border: showTerrain ? `1px solid rgba(82,71,184,0.3)` : "1px solid transparent", cursor: "pointer", boxShadow: "0 1px 4px rgba(0,0,0,0.06)", color: showTerrain ? ACCENT : "#475569", fontWeight: showTerrain ? 600 : 400 }}>
              <input type="checkbox" checked={showTerrain} onChange={e => setShowTerrain(e.target.checked)} style={{ accentColor: ACCENT }} /> 🏔 Terrain
            </label>
          )}
        </div>

        {hasTerrain && showTerrain && <SlopeLegend />}

        {(activeTool === "place_portail" || activeTool === "place_tree") && (
          <div style={{ position: "absolute", top: 60, left: "50%", transform: "translateX(-50%)", zIndex: 20, background: "rgba(82,71,184,0.92)", color: "white", padding: "6px 14px", borderRadius: 20, fontSize: 11, fontWeight: 600, boxShadow: "0 2px 8px rgba(0,0,0,0.15)", pointerEvents: "none" }}>
            {activeTool === "place_portail" ? "🚪 Cliquez pour placer le portail" : "🌳 Cliquez pour placer un arbre"}
          </div>
        )}
      ...
      <div style={{ width: 290, flexShrink: 0, display: "flex", flexDirection: "column", gap: 8, padding: 10, overflowY: "auto", background: "#f8fafc", borderLeft: "1px solid #e2e8f0" }}>
        ...
        {selectedBuilding ? (
          <BuildingPropertiesPanel
            ...
            onUpdateRenderSpec={(spec: BuildingBlenderSpec) => updateBuilding(selectedId!, { renderSpec: spec })}
            onDelete={() => deleteBuilding(selectedId!)}
            onDuplicate={() => duplicateBuilding(selectedId!)}
            onApplyTemplate={(type: BuildingTemplateType) => {
              const patch = patchFromTemplateType(type);
              updateLevels(selectedId!, patch.levels);
              updateStyle(selectedId!, patch.style);
            }}
            onSetTool={setTool}
          />
        ) : (
          <div style={{ background: "white", borderRadius: 12, padding: 20, border: "1px solid #e2e8f0", textAlign: "center", color: "#94a3b8", fontSize: 12 }}>
            ...
            {!has2DData && <div style={{ fontSize: 11, marginTop: 4, opacity: 0.7 }}>ou utilisez "Ajouter"</div>}
          </div>
        )}

        <div style={{ borderTop: "1px solid #e2e8f0", margin: "2px 0" }} />
        <VegetationPanel vegetation={vegetation} onChange={setVegetation} />
