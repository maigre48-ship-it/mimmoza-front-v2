...
type AppShellProps = { currentSpace: Space; onChangeSpace: (space: Space) => void; children: ReactNode; };
type NavItem = { label: string; path: string; icon: ComponentType<{ className?: string }>; end?: boolean; tabParam?: string; };
type NavSection = { id: string; label: string; items: NavItem[]; };
type StoredAccount = { email: string; fullName?: string; initials?: string; plan?: "free" | "starter" | "pro"; isAuthenticated: boolean; };
type PendingUnlockState = { path: string; dealId: string; dealLabel: string; };

const AUTH_STORAGE_KEY = "mimmoza.auth.v1";

// ══════════════════════════════════════════════════════════════════
// LOGO CUBE ISOMÉTRIQUE — intégré directement, aucun fichier SVG
// ══════════════════════════════════════════════════════════════════

function MimmozaLogo({ variant = "dark", iconOnly = false, width }: {
  variant?: "dark" | "white";
  iconOnly?: boolean;
  width?: number;
}) {
  ...
  if (iconOnly) {
    ...
    return (
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 42 48" width={pw} height={ph} role="img" aria-label="Mimmoza">
        <defs>
          <linearGradient id={`iT-${uid}`} x1="0%" y1="0%" x2="0%" y2="100%"><stop offset="0%" stopColor={topA}/><stop offset="100%" stopColor={topB}/></linearGradient>
          <linearGradient id={`iL-${uid}`} x1="100%" y1="0%" x2="0%" y2="0%"><stop offset="0%" stopColor={lefA}/><stop offset="100%" stopColor={lefB}/></linearGradient>
          <linearGradient id={`iR-${uid}`} x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor={rigA}/><stop offset="100%" stopColor={rigB}/></linearGradient>
        </defs>
  ...
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 240 60" width={pw} height={ph} role="img" aria-label="Mimmoza — Intelligence immobilière">
      <defs>
        <linearGradient id={`T-${uid}`} x1="0%" y1="0%" x2="0%" y2="100%"><stop offset="0%" stopColor={topA}/><stop offset="100%" stopColor={topB}/></linearGradient>
        <linearGradient id={`L-${uid}`} x1="100%" y1="0%" x2="0%" y2="0%"><stop offset="0%" stopColor={lefA}/><stop offset="100%" stopColor={lefB}/></linearGradient>
        <linearGradient id={`R-${uid}`} x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor={rigA}/><stop offset="100%" stopColor={rigB}/></linearGradient>
      </defs>
      ...
      <text x="61" y="42" fontFamily="-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif" fontWeight="400" fontSize="10.5" fill={tColor} letterSpacing="1.3">Intelligence immobilière</text>
    </svg>
  );
}
...
function getSpaceGradient(space: Space): string {
  switch (space) {
    case "marchand": return "linear-gradient(90deg, #2196f3 0%, #21cbf3 100%)";
    case "promoteur": return "linear-gradient(90deg, #7c6fcd 0%, #b39ddb 100%)";
    case "banque": return "linear-gradient(90deg, #26a69a 0%, #80cbc4 100%)";
    default: return "linear-gradient(90deg, #2196f3 0%, #21cbf3 100%)";
  }
}

function getSpaceAccentColor(space: Space): string {
  switch (space) {
    case "marchand": return "#1a72c4";
    case "promoteur": return "#5247b8";
    case "banque": return "#1a7a50";
    default: return "#1a72c4";
  }
}
...
function readStoredAccount(): StoredAccount | null {
  try {
    ...
    return { email: parsed.email, fullName: parsed.fullName ?? "", initials: parsed.initials ?? buildInitials(parsed.fullName, parsed.email), plan: parsed.plan ?? "free", isAuthenticated: true };
  } catch { return null; }
}
...
function getDealLabel(deal: MarchandDeal): string {
  return deal.title?.trim() || deal.address?.trim() || [deal.zipCode, deal.city].filter(Boolean).join(" ").trim() || deal.id;
}
...
function getPromoteurActiveSection(pathname: string): string | null {
  ...
  const prefixMap: [string, string][] = [
    ["/promoteur/foncier", "foncier"], ["/promoteur/plu-faisabilite", "foncier"], ["/promoteur/faisabilite", "foncier"],
    ["/promoteur/implantation-2d", "conception"], ["/promoteur/plan-2d", "conception"], ["/promoteur/massing-3d", "conception"], ["/promoteur/generateur-facades", "conception"],
    ["/promoteur/estimation", "evaluation"],
    ["/promoteur/marche", "etudes"], ["/promoteur/risques", "etudes"],
    ["/promoteur/bilan-promoteur", "bilan"], ["/promoteur/bilan", "bilan"], ["/promoteur/synthese", "bilan"], ["/promoteur/exports", "bilan"],
  ];
...
const SPACES: { id: Space; label: string; shortLabel: string; description: string; icon: ComponentType<{ className?: string }>; path: string; }[] = [
  { id: "marchand", label: "Espace Investisseur particulier et marchand de bien", shortLabel: "Investisseur", description: "Opportunités, scoring, rentabilité, exécution et sortie", icon: PieChart, path: "/marchand-de-bien" },
  ...
  { id: "banque", label: "Espace Financeur", shortLabel: "Financeur", description: "Analyse de risque, comité crédit, tarification et suivi dossiers", icon: ShieldCheck, path: "/banque" },
];

const SPACE_NAVIGATION: Record<Space, NavSection[]> = {
  none: [],
  promoteur: [
    { id: "demarrer", label: "Démarrer", items: [{ label: "Tableau de bord", path: "/promoteur", icon: BarChart3, end: true }] },
    ...
    { id: "conception", label: "Conception", items: [
      ...
      { label: "Générateur de façades", path: "/promoteur/generateur-facades", icon: Wand2 },
    ]},
    { id: "evaluation", label: "Évaluation", items: [{ label: "Estimation", path: "/promoteur/estimation", icon: TrendingUp }] },
    { id: "etudes", label: "Études", items: [
  ...
  marchand: [
    ...
    },
    {
      ...
      label: "Analyse",
      items: [
        { label: "Rentabilité",      path: "/marchand-de-bien/analyse", icon: PieChart,  end: true, tabParam: "rentabilite"    },
        { label: "Due Diligence",    path: "/marchand-de-bien/analyse", icon: FileText,             tabParam: "due_diligence"  },
        { label: "Marché / Risques", path: "/marchand-de-bien/analyse", icon: BarChart3,            tabParam: "marche_risques" },
        { label: "Synthèse IA",      path: "/marchand-de-bien/analyse", icon: Sparkles,             tabParam: "synthese_ia"    },
      ],
    },
  ],
  banque: [],
...
function TopNavigation({ currentSpace, onChangeSpace, isAdmin, onProtectedNavigate }: {
  currentSpace: Space; onChangeSpace: (space: Space) => void; isAdmin: boolean; onProtectedNavigate: (targetPath: string) => Promise<void>;
}) {
  ...
  const buildPath = (targetPath: string): string => preserveStudyInPath(targetPath, location.search);
  ...
  const resolvePath = (targetPath: string): string => {
    ...
    if (targetPath === "/banque") return activeId ? buildPath(`/banque/dossier/${activeId}`) : buildPath("/banque/dossiers");
    if (needsId.has(targetPath)) return !activeId ? buildPath("/banque/dossiers") : buildPath(`${targetPath}/${activeId}`);
  ...
  const activeSection = useMemo(() => {
    ...
    if (currentSpace === "promoteur") { const explicit = getPromoteurActiveSection(p); if (explicit) return explicit; }
    for (const section of spaceSections) {
      for (const item of section.items) {
        if (item.end ? p === item.path : p === item.path || p.startsWith(`${item.path}/`)) return section.id;
      }
    }
  ...
  const isActivePath = (targetPath: string, end?: boolean, tabParam?: string) => {
    ...
    if (tabParam !== undefined) {
      ...
      return (sp.get("tab") ?? "rentabilite") === tabParam;
    }
    return true;
  };

  const activeItems = spaceSections.find((s) => s.id === activeSection)?.items ?? [];
  const handleSpaceClick = (space: Space, path: string) => (e: React.MouseEvent<HTMLAnchorElement>) => { e.preventDefault(); onChangeSpace(space); navigate(path); };
  const handleAccountClick = () => navigate(account?.isAuthenticated ? "/compte" : "/connexion");
      ...
      <div className="border-b border-slate-200/70 bg-white/90 backdrop-blur-md">
        <div className="mx-auto max-w-7xl px-4 lg:px-6">
          <div className="flex h-16 items-center justify-between gap-4">
            ...
            {/* ── LOGO ── */}
            <NavLink to="/" onClick={() => onChangeSpace("none")}
              className="flex shrink-0 items-center rounded-xl px-2 py-1.5 transition-colors hover:bg-slate-100/80">
            ...
            <div className="flex flex-1 items-center justify-center">
              <nav className="flex items-center gap-1.5 overflow-x-auto rounded-2xl border border-slate-200/80 bg-slate-50/80 p-1.5 scrollbar-hide">
                {SPACES.map((space) => {
                  ...
                  return (
                    <a key={space.id} href={space.path} onClick={handleSpaceClick(space.id, space.path)}
                      ...
                      style={isActive ? { background: grad, color: "white" } : { color: "#64748b" }}>
                ...
                <NavLink to="/veille" onClick={() => onChangeSpace("none")}
                  className="group relative flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-medium whitespace-nowrap transition-all"
                  style={({ isActive }) => isActive ? { background: "linear-gradient(90deg, #1e3a5f 0%, #1a3060 100%)", color: "white" } : { color: "#64748b" }}>
                  {({ isActive }) => (<><Eye className={`h-4 w-4 transition-colors ${isActive ? "text-white" : "text-slate-400 group-hover:text-slate-700"}`} /><span>Veille</span></>)}
                </NavLink>
                <NavLink to="/api" onClick={() => onChangeSpace("none")}
                  className="group relative flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-medium whitespace-nowrap transition-all"
                  style={({ isActive }) => isActive ? { background: "linear-gradient(90deg, #6366f1 0%, #8b5cf6 100%)", color: "white" } : { color: "#64748b" }}>
                  {({ isActive }) => (<><Code2 className={`h-4 w-4 transition-colors ${isActive ? "text-white" : "text-slate-400 group-hover:text-slate-700"}`} /><span>API</span></>)}
                </NavLink>
              </nav>
            </div>
            ...
            <div className="flex shrink-0 items-center gap-2">
              ...
              <button type="button" onClick={handleAccountClick}
                ...
                <div className="flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br from-slate-100 to-slate-200 text-[11px] font-semibold text-slate-600">{account?.initials ?? "AM"}</div>
                <div className="hidden md:block leading-none pr-1 text-left">
                  <div className="text-xs font-medium text-slate-700">{account?.fullName?.trim() || (account?.isAuthenticated ? account.email : "Mon compte")}</div>
                  <div className="mt-0.5 text-[10px] text-slate-400">{account?.isAuthenticated ? (account.plan === "pro" ? "Abonnement Pro" : account.plan === "starter" ? "Abonnement Starter" : "Compte gratuit") : "Connexion / inscription"}</div>
                </div>
              </button>
            </div>
          </div>
          ...
          <div className="mx-auto max-w-7xl px-4 lg:px-6">
            <div className="flex items-center justify-between border-b border-slate-100 py-3">
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="text-slate-500">{spaceSections.find((s) => s.id === activeSection)?.label}</span>
                {studyId && (
                  <>
                    <ChevronRight className="h-3.5 w-3.5 text-slate-300" />
                    <span className="inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-xs font-medium" style={{ background: `${spaceAccent}18`, color: spaceAccent }} title={studyId}>
                      <Sparkles className="h-3 w-3" />{studyId.length > 10 ? `${studyId.slice(0, 10)}…` : studyId}
                    </span>
                  </>
                )}
              </div>
              <button type="button" onClick={() => { onChangeSpace("none"); navigate("/"); }}
                ...
                <Home className="h-3.5 w-3.5" /><span className="hidden sm:inline">Accueil</span>
              </button>
            </div>
            ...
            {spaceSections.length > 1 && (
              <div className="flex items-center gap-1 py-2 overflow-x-auto scrollbar-hide">
                {spaceSections.map((section) => {
                  ...
                  const resolvedBase = resolvePath(firstItem?.path ?? "");
                  const resolvedPath = buildNavItemPath(resolvedBase, firstItem?.tabParam);
                  return (
                    <a key={section.id} href={resolvedPath} onClick={(e) => { e.preventDefault(); void onProtectedNavigate(resolvedPath); }}
                      className="relative rounded-xl px-4 py-2 text-sm font-medium whitespace-nowrap transition-all"
                      style={active ? { background: spaceGradient, color: "white" } : { color: "#64748b" }}>
                      {section.label}
                    </a>
                  );
                })}
              ...
              <div className={spaceSections.length > 1 ? "border-t border-slate-100 py-2" : "py-2"}>
                <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide">
                  {activeItems.map((item) => {
                    ...
                    return (
                      <a key={`${item.path}-${item.tabParam ?? "default"}`} href={resolvedPath}
                        onClick={(e) => { e.preventDefault(); void onProtectedNavigate(resolvedPath); }}
                        className={["inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-medium whitespace-nowrap transition-all",
                          active ? "border border-slate-200 bg-white text-slate-950 shadow-sm" : "text-slate-500 hover:bg-white/70 hover:text-slate-700"].join(" ")}>
                        <Icon className="h-4 w-4" style={{ color: active ? spaceAccent : "#94a3b8" }} />
...
function MobileDrawer({ isOpen, onClose, currentSpace, onChangeSpace, isAdmin, onProtectedNavigate }: {
  isOpen: boolean; onClose: () => void; currentSpace: Space; onChangeSpace: (space: Space) => void;
  isAdmin: boolean; onProtectedNavigate: (targetPath: string, closeAfter?: boolean) => Promise<void>;
}) {
  const location = useLocation();
  const navigate = useNavigate();
  const account = useMemo(() => readStoredAccount(), [location.pathname, location.search]);
  ...
  const isPathActive = (tp: string, end?: boolean, tabParam?: string) => {
    ...
    if (!pathMatch) return false;
    if (tabParam !== undefined) { const sp = new URLSearchParams(location.search); return (sp.get("tab") ?? "rentabilite") === tabParam; }
    return true;
  };
  ...
  const handleSelectSpace = (space: Space) => { const meta = SPACES.find((s) => s.id === space); onChangeSpace(space); if (meta) navigate(meta.path); onClose(); };
  const handleNavClick = (item: NavItem) => (e: React.MouseEvent<HTMLAnchorElement>) => { e.preventDefault(); void onProtectedNavigate(buildNavItemPath(resolvePath(item.path), item.tabParam), true); };
  const handleAccountClick = () => { navigate(account?.isAuthenticated ? "/compte" : "/connexion"); onClose(); };
    ...
    <div className="fixed inset-0 z-50 flex md:hidden">
      <div className="flex w-80 max-w-[85%] flex-col bg-white shadow-2xl animate-slide-in">
        <div className="flex items-center justify-between border-b border-slate-100 px-4 py-4">
          ...
          <button type="button" onClick={onClose} className="rounded-lg p-2 transition-colors hover:bg-slate-100"><X className="h-4 w-4 text-slate-500" /></button>
        </div>
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto px-3 py-4">
          <button type="button" onClick={handleAccountClick} className="mb-3 flex w-full items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-3 text-left transition-all hover:bg-slate-100">
            <div className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-slate-100 to-slate-200 text-xs font-semibold text-slate-600">{account?.initials ?? "AM"}</div>
            <div className="min-w-0">
              <div className="truncate text-sm font-semibold text-slate-800">{account?.fullName?.trim() || (account?.isAuthenticated ? account.email : "Mon compte")}</div>
              <div className="mt-0.5 text-xs text-slate-500">{account?.isAuthenticated ? "Accéder à votre compte" : "Connexion / inscription"}</div>
            </div>
          </button>
          ...
          {isAdmin && (
            <button type="button" onClick={() => { navigate("/admin"); onClose(); }} className="mb-3 flex w-full items-center gap-3 rounded-2xl border border-slate-200 bg-white px-3 py-3 text-left transition-all hover:bg-slate-50">
              <div className="flex h-10 w-10 items-center justify-center rounded-full bg-slate-100 text-slate-600"><LayoutDashboard className="h-5 w-5" /></div>
              <div className="min-w-0">
          ...
          {currentSpace !== "none" && allNavItems.length > 0 && (
            <>
              <div className="px-3 py-2 text-xs font-semibold uppercase tracking-wider text-slate-400">{SPACES.find((s) => s.id === currentSpace)?.label}</div>
              {allNavItems.map((item) => {
                const Icon = item.icon;
                const isActive = isPathActive(item.path, item.end, item.tabParam);
                return (
                  <a key={`${item.path}-${item.tabParam ?? "default"}`} href={buildNavItemPath(resolvePath(item.path), item.tabParam)} onClick={handleNavClick(item)}
                    className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 transition-all"
                    style={isActive ? { background: spaceGradient, color: "white" } : { color: "#374151" }}>
          ...
          {SPACES.filter((s) => s.id !== currentSpace).map((space) => {
            ...
            return (
              <button key={space.id} type="button" onClick={() => handleSelectSpace(space.id)} className="flex w-full flex-col rounded-xl px-3 py-2.5 text-left text-slate-700 transition-all hover:bg-slate-50">
                <span className="flex items-center gap-3"><Icon className="h-4 w-4 text-slate-400" /><span className="text-sm font-medium">{space.label}</span></span>
          ...
          <div className="mt-4 border-t border-slate-100 pt-4">
            ...
            <NavLink to="/veille" onClick={onClose} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 transition-all"
              style={({ isActive }) => isActive ? { background: "linear-gradient(90deg, #1e3a5f 0%, #1a3060 100%)", color: "white" } : { color: "#374151" }}>
              {({ isActive }) => (<><Eye className={`h-4 w-4 ${isActive ? "text-white" : "text-blue-400"}`} /><span className="text-sm font-medium">Veille marché</span></>)}
            </NavLink>
            <NavLink to="/api" onClick={onClose} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 transition-all"
              style={({ isActive }) => isActive ? { background: "linear-gradient(90deg, #6366f1 0%, #8b5cf6 100%)", color: "white" } : { color: "#374151" }}>
              {({ isActive }) => (<><Code2 className={`h-4 w-4 ${isActive ? "text-white" : "text-indigo-400"}`} /><span className="text-sm font-medium">API</span></>)}
            </NavLink>
          </div>
          ...
          <div className="mt-4 border-t border-slate-100 pt-4">
            <NavLink to="/" onClick={() => { onChangeSpace("none"); onClose(); }} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-slate-600 hover:bg-slate-50">
              <Home className="h-4 w-4 text-slate-500" /><span>Retour à l&apos;accueil</span>
            </NavLink>
          </div>
        </nav>
      </div>
...
export function AppShell({ currentSpace, onChangeSpace, children }: AppShellProps) {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(() => { void refreshAdminStatus(); });
    return () => { mounted = false; subscription.unsubscribe(); };
  }, []);

  const closeUnlockModal = () => { setPendingUnlock(null); setUnlockLoading(false); setUnlockNoTokens(false); setUnlockError(null); };

  const handleProtectedNavigate = async (targetPath: string, closeAfterMobile = false): Promise<void> => {
    if (closeAfterMobile) setMobileNavOpen(false);
    if (currentSpace !== "marchand" || !isMarchandPremiumPath(targetPath)) { navigate(targetPath); return; }
    const deal = ensureActiveDeal();
    if (!deal) { navigate("/marchand-de-bien"); return; }
    if (deal.premiumUnlocked) { navigate(targetPath); return; }
    setUnlockError(null); setUnlockNoTokens(false);
  ...
  const confirmUnlock = async () => {
    ...
    try {
      setUnlockLoading(true); setUnlockError(null); setUnlockNoTokens(false);
      const result = await unlockDealIfNeeded(pendingUnlock.dealId);
      if (!result.ok) { if (result.reason === "NO_TOKENS") { setUnlockNoTokens(true); return; } setUnlockError(result.message); return; }
      const nextPath = pendingUnlock.path; closeUnlockModal(); navigate(nextPath);
    } catch (error) { console.error("Erreur déverrouillage deal:", error); setUnlockError("Impossible de déverrouiller ce projet pour le moment."); }
    finally { setUnlockLoading(false); }
  };

  const openBilling       = () => { closeUnlockModal(); navigate("/compte"); };
  const openSubscriptions = () => { closeUnlockModal(); navigate("/compte?section=abonnements"); };
  const openTokens        = () => { closeUnlockModal(); navigate("/compte?section=jetons"); };
  ...
  return (
    <div className="flex min-h-screen flex-col bg-slate-50 text-slate-900">
      ...
      <DealUnlockModal open={Boolean(pendingUnlock)} dealLabel={pendingUnlock?.dealLabel ?? "Projet"} loading={unlockLoading} noTokens={unlockNoTokens} errorMessage={unlockError}
        onClose={closeUnlockModal} onConfirmUnlock={confirmUnlock} onOpenBilling={openBilling} onOpenSubscriptions={openSubscriptions} onOpenTokens={openTokens} />
    </div>
  );
}