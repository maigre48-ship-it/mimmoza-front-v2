// ──────────────────────────────────────────────────────────────────────────────
// _shared/predictive/ecb.ts
// TAUX DIRECTEURS BCE — logique partagée front / edge functions
// ──────────────────────────────────────────────────────────────────────────────
//
// Pourquoi ce fichier existe
// --------------------------
// Le service d'origine (src/.../ecbRate.service.ts) mêlait DEUX choses : la
// logique BCE (récupération, analyse, repli) et un cache adossé à
// `userStorage`, donc au localStorage du navigateur. Cette seconde partie
// interdisait de le réutiliser côté Deno, où localStorage n'existe pas.
//
// La logique est donc ici, SANS cache — chaque environnement gère le sien :
//   • le front conserve son cache localStorage de 12 h ;
//   • les edge functions utilisent un cache mémoire par instance.
//
// Une seule implémentation de la lecture BCE, deux stratégies de cache là où
// elles ont un sens. Toute correction du calcul de pression ou de tendance se
// fait ICI et profite aux deux côtés.
//
// Ne sert PAS à afficher « Taux BCE = X % » : sert à produire une pression
// crédit, une tendance et une phrase métier exploitables par le moteur.
// ──────────────────────────────────────────────────────────────────────────────
// ──────────────────────────────────────────────────────────────────────────────
// ecbRate.service.ts
...
// ── API publique (sans cache) ────────────────────────────────────────────────

/**
 * Récupère et analyse les taux directeurs BCE.
 *
 * PAS de cache : l'appelant en pose un s'il en veut un. Le repli est interne —
 * cette fonction ne rejette jamais, elle renvoie une analyse marquée
 * `source: "fallback"` quand l'API BCE est injoignable. L'appelant DOIT
 * relayer cette distinction : un taux de repli n'est pas un taux constaté.
 */
export async function fetchEcbRatesAnalysis(): Promise<EcbRatesAnalysis> {
  try {
    const rates = await fetchAllRates();
    return buildAnalysis(rates, "ecb");
  } catch {
