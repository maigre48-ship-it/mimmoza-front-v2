...
/**
 * Impact des dispositifs d'investissement locatif, calculé et non forfaitisé.
 *
 * Le bonus renvoyé est l'avantage fiscal TOTAL sur la durée d'engagement,
 * rapporté au prix d'acquisition, exprimé en points. C'est une grandeur
 * homogène au reste de l'échelle (elle multiplie le profit net) et, surtout,
 * elle est reconstituable : chaque euro vient du moteur des dispositifs, donc
 * d'un texte fiscal daté.
 *
 * Retourne null si le régime n'est pas un dispositif, laissant la main au
 * switch historique des régimes (LMNP, SCI, nu…).
 */
function resolveDispositifImpact(
  fiscalite: PredictiveFiscalite,
  tmi: number,
  acquisitionPrice?: number
): { bonus: number; label: string; hasData: boolean } | null {
  const regime = fiscalite.regime;
  if (
    regime !== "jeanbrun_neuf" &&
    regime !== "jeanbrun_ancien" &&
    regime !== "denormandie" &&
    regime !== "loc_avantages"
  ) {
    return null;
  }

  const prix = typeof acquisitionPrice === "number" && acquisitionPrice > 0
    ? acquisitionPrice
    : 0;
  if (prix === 0) {
    return {
      bonus: 0,
      label: "Dispositif renseigné mais prix d'acquisition manquant : impact non chiffré",
      hasData: true,
    };
  }

  const niveauLoyer = fiscalite.niveauLoyer ?? "intermediaire";
  const logement = {
    prixAcquisitionNetFraisEur: prix,
    travauxEur: fiscalite.travauxEur,
  };
  const situation = { tmiPct: tmi };
  ...
  switch (regime) {
    ...
    case "jeanbrun_ancien":
      ...
      break;
    case "denormandie":
      resultat = calculerDenormandie({ logement, situation, dureeEngagementAns: 9 });
      break;
    default:
      resultat = calculerLocAvantages({
        ...
        situation,
        niveauLoyer,
        intermediationLocative: fiscalite.intermediationLocative,
      });
      break;
  }

  // Un dispositif dont une condition est bloquante ne rapporte rien : le
  // signaler vaut mieux que d'afficher un gain que l'investisseur n'aura pas.
  const bloquants = resultat.constats.filter((c) => c.niveau === "bloquant");
  if (bloquants.length > 0) {
    return {
      bonus: 0,
      label: `${resultat.libelle} — conditions non remplies : ${bloquants[0].message}`,
      hasData: true,
    };
  }

  const bonus = parseFloat(((resultat.avantageTotalEur / prix) * 100).toFixed(1));
  return {
    bonus,
    label:
      `${resultat.libelle} — ${resultat.avantageTotalEur.toLocaleString("fr-FR")} € d'avantage ` +
      `sur ${resultat.dureeEngagementAns} ans, soit ${bonus} points du prix d'acquisition ` +
      `(TMI ${tmi} %, barèmes ${resultat.millesimeBaremes})`,
    hasData: true,
  };
}
...
function resolveFiscalImpact(
  ...
  // Les dispositifs d'investissement locatif ne reçoivent PAS un bonus
  // forfaitaire : leur impact est calculé par le moteur des dispositifs, puis
  // rapporté au prix d'acquisition. Le nombre de points obtenu est donc un
  // vrai résultat, traçable jusqu'au texte fiscal.
  const bonusDispositif = resolveDispositifImpact(fiscalite, tmi, acquisitionPrice);
  if (bonusDispositif) return bonusDispositif;
  ...
  switch (fiscalite.regime) {
    ...
    case "pinel":
      // Le Pinel n'est plus souscriptible depuis le 01/01/2025. Le bonus
      // forfaitaire de +20 points qui figurait ici valorisait un dispositif
      // fermé — et contredisait le reste du produit, qui annonce sa clôture.
      // Un snapshot ancien peut encore porter cette valeur : on la reconnaît,
      // on l'explique, on ne la récompense pas.
      return {
        bonus: 0,
        label: "Pinel — dispositif clos aux nouveaux investissements depuis le 01/01/2025 (successeur : Jeanbrun)",
        hasData: true,
      };
...
function resolveMarketScores(input: PredictiveEngineInput): {
  ...
  // Géorisques : chaque risque identifié aggrave le niveau de risque de 5 pts
  const { risques } = resolveGeorisquesAdjustment(input.georisques);
  const niveauRisque = clamp(baseRisk + risques.length * 5, 15, 92);

  // ⚠️ CONVENTION — `niveauRisque` monte quand le risque augmente. Ce moteur
  // exposait cette valeur telle quelle sous le nom `riskScore`, et l'écran
  // l'affichait brute (« Risque : 85/100 ») juste à côté de « Liquidité :
  // 70/100 », où 100 est bon. Partout ailleurs chez Mimmoza — riskDisplay.ts,
  // Bilan promoteur, opportunityEngine, et jusqu'à la règle 4quinquies du
  // prompt du copilote — 100 signifie « très sûr ». Un terrain criblé de
  // géorisques affichait donc un score élevé, lu comme une bonne nouvelle.
  // On repasse dans la convention maison au moment de sortir du moteur.
  const securityScore = clamp(100 - niveauRisque, 0, 100);

  return { pressureScore, liquidityScore, securityScore };
}
...
export function computePredictiveSnapshot(
  ...
  const { pressureScore, liquidityScore, securityScore } = resolveMarketScores(input);
  ...
  const { bonus: fiscalBonus } = resolveFiscalImpact(
    input.fiscalite,
    input.rentabilite?.rendementBrut,
    input.acquisitionPrice
  );
  ...
  // L'avantage fiscal AMÉLIORE le résultat, quel que soit son signe.
  //
  // La formule précédente multipliait le résultat brut par (1 + bonus/100).
  // Sur une opération bénéficiaire, cela fonctionnait. Sur une opération en
  // perte, le résultat étant négatif, le multiplicateur CREUSAIT la perte :
  // une opération à −251 000 € était affichée à −268 570 € parce que
  // l'investisseur bénéficiait d'un dispositif fiscal. L'absurdité était
  // masquée tant que le bonus restait forfaitaire.
  //
  // On applique donc le bonus à la VALEUR ABSOLUE du résultat : le gain va
  // toujours dans le sens favorable. Pour un résultat positif, le calcul est
  // rigoureusement identique à l'ancien.
  const resultatBrut       = target12 - totalCost;
  const projectedNetProfit = Math.round(
    resultatBrut + Math.abs(resultatBrut) * (fiscalBonus / 100),
  );
  ...
  return {
    ...
    market: { regime, pressureScore, liquidityScore, securityScore },
    forecast,
    scenarios,
    drivers,
    operationImpact,
