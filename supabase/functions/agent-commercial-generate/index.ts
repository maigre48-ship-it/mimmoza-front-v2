...
// ------------------------------
// La génération ne recevait AUCUN historique : le prompt était identique qu'il
// s'agisse d'un premier contact ou d'une troisième relance. Or quatre des sept
// types d'emails sont des suites — relance_1, relance_2, reponse_question,
// suivi_demo. On demandait littéralement à relance_1 d'« apporter un angle
// nouveau » alors que le modèle n'avait jamais vu le message précédent : il ne
// pouvait pas savoir quel angle avait déjà servi. Ces types produisaient donc
// structurellement du générique, et pouvaient répéter mot pour mot un message
// déjà reçu par le prospect.
//
// Deux blocs distincts, et la distinction n'est pas cosmétique
// ------------------------------------------------------------
//   • ENVOYÉS : ce que le prospect a réellement lu. C'est ce qu'il ne faut pas
//     répéter, et ce à quoi le nouveau message doit faire suite.
//   • REFUSÉS : des brouillons que le relecteur humain a écartés, JAMAIS partis.
//     Les taire ferait reproposer un angle déjà jugé mauvais ; les mélanger aux
//     envoyés ferait écrire « comme je vous l'écrivais » à propos d'un message
//     que le prospect n'a jamais vu. D'où deux sections explicitement séparées.
//
// Les brouillons en attente de relecture (pending_review) sont volontairement
// EXCLUS : ils ne sont ni envoyés ni refusés, les inclure n'apprendrait rien et
// risquerait de faire croire à un envoi.
const HISTORIQUE_MAX_MESSAGES = 6;
const HISTORIQUE_MAX_CORPS = 1200;

async function buildHistoriqueContext(
  admin: SupabaseClient,
  prospectId: string,
): Promise<string> {
  const { data, error } = await admin
    .from('commercial_emails')
    .select('kind, subject, body, internal_rationale, status, sent_at, created_at')
    .eq('prospect_id', prospectId)
    .in('status', ['sent', 'approved', 'rejected'])
    .order('created_at', { ascending: true })
    .limit(HISTORIQUE_MAX_MESSAGES * 2);

  // L'historique est un CONFORT : son échec ne doit pas empêcher de rédiger.
  // Mais il ne doit pas non plus passer pour une absence d'historique — d'où
  // un message explicite plutôt qu'une chaîne vide.
  if (error) {
    return '(Historique indisponible : la lecture des échanges précédents a échoué. ' +
           "Ne fais AUCUNE référence à un message antérieur, tu n'as pas pu le lire.)";
  }

  const lignes = (data ?? []) as Array<{
    kind: string; subject: string | null; body: string | null;
    internal_rationale: string | null; status: string;
    sent_at: string | null; created_at: string;
  }>;

  const envoyes = lignes.filter((l) => l.status === 'sent' || l.status === 'approved')
                        .slice(-HISTORIQUE_MAX_MESSAGES);
  const refuses = lignes.filter((l) => l.status === 'rejected').slice(-3);

  if (envoyes.length === 0 && refuses.length === 0) {
    return "(Aucun échange antérieur avec ce prospect : c'est un premier contact. " +
           'Ne fais référence à AUCUN message précédent, à aucune conversation, à aucun ' +
           'rendez-vous — il n\'y en a pas eu.)';
  }

  const jour = (iso: string | null) => (iso ? iso.slice(0, 10) : 'date inconnue');
  const tronque = (t: string | null) =>
    !t ? '(vide)'
       : t.length > HISTORIQUE_MAX_CORPS ? `${t.slice(0, HISTORIQUE_MAX_CORPS)}… [tronqué]` : t;

  const parts: string[] = [];

  if (envoyes.length > 0) {
    parts.push(
      `### Messages DÉJÀ ENVOYÉS à ce prospect (${envoyes.length}) — il les a reçus\n` +
      envoyes.map((e, i) =>
        `[${i + 1}] ${jour(e.sent_at ?? e.created_at)} · type ${e.kind}\n` +
        `Objet : ${e.subject ?? '(sans objet)'}\n${tronque(e.body)}`
      ).join('\n\n'),
    );
  } else {
    parts.push('### Messages déjà envoyés : AUCUN — rien ne lui est encore parti.');
  }

  if (refuses.length > 0) {
    parts.push(
      `### Angles REFUSÉS par le relecteur (${refuses.length}) — JAMAIS envoyés, le prospect ne les a PAS vus\n` +
      refuses.map((e) =>
        `- « ${e.subject ?? '(sans objet)'} » (${e.kind})` +
        (e.internal_rationale ? `\n  Intention d'alors : ${e.internal_rationale}` : '')
      ).join('\n') +
      '\nNe reprends pas ces angles, et n\'y fais aucune allusion : ils n\'existent pas pour le prospect.',
    );
  }
...
async function callAnthropic(
  ...
  const system =
    ...
    "HISTORIQUE : un bloc te donne les messages DÉJÀ ENVOYÉS à ce prospect. " +
    "Tu ne dois JAMAIS réutiliser un angle, une accroche ou une formulation qui s'y trouve : " +
    "recevoir deux fois le même argument est la façon la plus sûre de perdre un prospect. " +
    "Une relance doit apporter quelque chose que le message précédent ne disait pas. " +
    "Si le bloc indique qu'aucun message n'a été envoyé, n'évoque AUCUN échange antérieur, " +
    "aucun appel, aucun rendez-vous : il n'y en a pas eu, et l'inventer se voit immédiatement. " +
    "Le bloc peut aussi lister des angles REFUSÉS par le relecteur : ceux-là ne sont jamais " +
    "partis, le prospect ne les a pas vus — ne les reprends pas et n'y fais pas allusion. " +
    "⚠️ Le système ne stocke AUCUNE réponse reçue du prospect. Si tu dois répondre à une " +
    "question et qu'elle ne t'est pas fournie explicitement, ne l'invente pas : rédige un " +
    "message court demandant une précision, et signale dans internal_rationale que la " +
    "question du prospect n'était pas disponible. " +
  ...
  const user =
    ...
    `- Statut actuel dans le pipeline : ${prospect.status}\n\n` +
    `Historique des échanges avec ce prospect :\n${historique}\n\n` +
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    //    Base de connaissances et historique sont indépendants : on les lit en
    //    parallèle plutôt que d'ajouter un aller-retour avant chaque rédaction.
    const [knowledge, historique] = await Promise.all([
      buildKnowledgeContext(admin),
      buildHistoriqueContext(admin, prospectId),
    ]);
    const result = await callAnthropic(
      prospect as ProspectRow, kind as EmailKind, knowledge, historique,
    );
